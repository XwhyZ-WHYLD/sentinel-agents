"""Feedback loop for Sentinel Agents Phase 6.

The `FeedbackLoop` watches execution outputs and extracts potentially risky
tokens.  If the output contains certain risky keywords, those tokens are
fed back into the policy engine to be blocked on future runs.  This
mechanism is extremely simple but demonstrates how a feedback loop can
encourage safer behaviour over time.
"""
