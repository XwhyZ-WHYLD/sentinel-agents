"""JSON‑backed memory store for execution traces.

This simple implementation persists a list of traces to a JSON file on disk.
Each trace is a dictionary containing at minimum a `task` and `result` field.
The store assigns a monotonically increasing identifier to each saved trace
and supports basic prefix search.  In a production system this module
could be replaced with a vector store or database.
"""
...
class MemoryStore:
    ...
    def save_trace(self, trace: Dict[str, Any]) -> None:
        ...
        trace = dict(trace)  # make a shallow copy
